...
- `useServerFn` â€” React hook for calling server functions from components
...
- `<RouterProvider>` â€” not needed in Start (handled automatically)
- `<Outlet>` â€” renders matched child route
- `<Link>` â€” type-safe navigation
- `<Navigate>` â€” declarative redirect
- `<HeadContent>` â€” renders head tags (must be in `<head>`)
- `<Scripts>` â€” renders body scripts (must be in `<body>`)
- `<Await>` â€” renders deferred data with Suspense
- `<ClientOnly>` â€” renders children only after hydration
- `<CatchBoundary>` â€” error boundary

## Hooks Reference

All hooks from `@tanstack/react-router` work in Start:

- `useRouter()` â€” router instance
- `useRouterState()` â€” subscribe to router state
- `useNavigate()` â€” programmatic navigation
- `useSearch({ from })` â€” validated search params
- `useParams({ from })` â€” path params
- `useLoaderData({ from })` â€” loader data
- `useMatch({ from })` â€” full route match
- `useRouteContext({ from })` â€” route context
- `Route.useLoaderData()` â€” typed loader data (preferred in route files)
- `Route.useSearch()` â€” typed search params (preferred in route files)
...
```tsx
// WRONG â€” this is the SPA router, NOT Start
import { createServerFn } from '@tanstack/react-router'

// CORRECT â€” server functions come from react-start
import { createServerFn } from '@tanstack/react-start'

// CORRECT â€” routing APIs come from react-router (re-exported by Start too)
...
```tsx
// WRONG â€” beforeLoad/loader are NOT React components
beforeLoad: () => {
  const auth = useAuth() // React hook, cannot be used here
}

// CORRECT â€” pass state via router context
...
- [start-core](../../../start-client-core/skills/start-core/SKILL.md) â€” core Start concepts
- [router-core](../../../router-core/skills/router-core/SKILL.md) â€” routing fundamentals
- [react-router](../../../react-router/skills/react-router/SKILL.md) â€” React Router hooks and components
